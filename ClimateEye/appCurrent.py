...
def generate_tiles(north, south, east, west, tile_size_km=0.5):
    """
    Legacy function for backward compatibility.
    
    Accepts directional parameter names (north/south/east/west) but internally uses
    coordinate-based geometry. Parameter names are just labels for coordinate boundaries.
    The actual calculation is purely coordinate-based and direction-agnostic.
    ...
    - List of tile dictionaries with center coordinates and bounds
    """
    # Convert directional parameter names to coordinate-based values
    # The calculation itself is direction-agnostic
    return generate_tiles_from_bounds(south, north, west, east, tile_size_km)
...
def get_aqi_tiles():
    ...
    try:
        ...
        use_0to3m = data.get('use_0to3m', True)  # Default to 0-3m height
        
        if not start_date or not end_date:
            return jsonify({'error': 'Start date and end date are required'}), 400
        
        # Handle polygon geometry (preferred)
        if polygon:
        ...
        # Handle bounding box (legacy support)
        elif bounds:
            ...
            if None in [north, south, east, west]:
                return jsonify({'error': 'All bound coordinates (north, south, east, west) are required'}), 400
            ...
            # Generate tiles (converts to polygon internally)
            tiles = generate_tiles(north, south, east, west, tile_size_km)
            bounds_data = {
                "min_lat": south,
                "max_lat": north,
                "min_lon": west,
            ...
            return jsonify({'error': 'Either polygon or bounds are required'}), 400
        ...
        for tile in tiles:
            try:
                ...
                # Calculate correction factor for 0-3m height
                # Use middle of date range at noon
                mid_date = start_date_obj + (end_date_obj - start_date_obj) / 2
                mid_datetime = datetime.combine(mid_date, datetime.min.time()).replace(
                    hour=12, minute=0, second=0, tzinfo=timezone.utc
                )
                cf_data = calculate_correction_factor(center_lat, center_lon, mid_datetime)
                cf = cf_data["cf"]
                
                # Calculate corrected values for 0-3m
                pm10_0to3m = round(avg_pm10 * cf, 2) if avg_pm10 is not None else None
                pm2_5_0to3m = round(avg_pm2_5 * cf, 2) if avg_pm2_5 is not None else None
                co_0to3m = round(avg_co * cf, 2) if avg_co is not None else None
                no2_0to3m = round(avg_no2 * cf, 2) if avg_no2 is not None else None
                so2_0to3m = round(avg_so2 * cf, 2) if avg_so2 is not None else None
                o3_0to3m = round(avg_o3 * cf, 2) if avg_o3 is not None else None
                
                # Calculate AQI at 0-3m height
                aqi_0to3m = calculate_aqi(
                    pm25=pm2_5_0to3m,
                    pm10=pm10_0to3m,
                    o3=o3_0to3m,
                    no2=no2_0to3m,
                ...
                # Use 0-3m AQI if requested, otherwise use 10m AQI
                display_aqi = int(aqi_0to3m) if (use_0to3m and aqi_0to3m is not None) else int(aqi)
                ...
                if aqi_0to3m is not None:
                    aqi_0to3m_values.append(aqi_0to3m)
        ...
        summary = {
            ...
            "avg_aqi_0to3m": round(sum(aqi_0to3m_values) / len(aqi_0to3m_values), 2) if aqi_0to3m_values else None,
            "min_aqi": int(min(aqi_values)) if aqi_values else None,
            "max_aqi": int(max(aqi_values)) if aqi_values else None,
            "min_aqi_0to3m": int(min(aqi_0to3m_values)) if aqi_0to3m_values else None,
            "max_aqi_0to3m": int(max(aqi_0to3m_values)) if aqi_0to3m_values else None
        }
...
@app.route('/api/test/height-correction', methods=['POST'])
def test_height_correction():
    """
    Step 1: Test endpoint to verify height correction function
    Tests the calculate_correction_factor function with sample data
    
    Expects:
    {
        "latitude": float,
        "longitude": float,
        "datetime": "YYYY-MM-DD HH:MM:SS" (optional, uses current time if not provided)
    }
    """
    try:
        data = request.get_json()
        ...
        # Test the correction factor calculation
        cf_data = calculate_correction_factor(latitude, longitude, dt)
        
        # Sample AQI values to test conversion
        sample_aqi_10m = 100  # AQI at 10m height
        sample_aqi_0to3m = round(sample_aqi_10m * cf_data["cf"], 2)
        
        return jsonify({
            "test_name": "Height Correction Factor Test",
            "location": {
                "latitude": latitude,
                "longitude": longitude
            },
            "datetime": dt.strftime('%Y-%m-%d %H:%M:%S'),
            "correction_factor_details": cf_data,
            "sample_conversion": {
                "aqi_at_10m": sample_aqi_10m,
                "correction_factor": cf_data["cf"],
                "aqi_at_0to3m": sample_aqi_0to3m,
                "increase_percentage": round((cf_data["cf"] - 1) * 100, 2)
            },
            "explanation": {
                "vpc": "Vertical Proximity Correction: Accounts for higher concentrations at ground level",
                "tdc": f"Time-of-Day Correction: {'Daytime (11am-6pm)' if cf_data['tdc'] == 1.1 else 'Night/Morning/Evening'}",
                "lsaf": "Location Specific Adjustment Factor: Accounts for local environmental factors",
                "hcf": f"Humidity Correction Factor: Based on relative humidity ({cf_data['rh']}%)"
            }
        }), 200
        
    except Exception as e:
        import traceback
        logger.error(f"Error in height correction test: {str(e)}")
        logger.error(traceback.format_exc())
        return jsonify({
            'error': f'Failed to test height correction: {str(e)}'
        }), 500

@app.route('/api/test/tiles', methods=['POST'])
def test_tile_generation():
    """
    Test endpoint to visualize tile generation without AQI calculations
    ...
    try:
        ...
        if polygon:
            ...
            # Calculate bounding box for summary
            bounds_data = get_polygon_bounds(polygon)
        ...
        import math
        if bounds_data:
            avg_lat = (bounds_data["min_lat"] + bounds_data["max_lat"]) / 2
            lat_km_per_deg = 111.0
            lon_km_per_deg = 111.0 * math.cos(math.radians(avg_lat))
            
            lat_range_km = (bounds_data["max_lat"] - bounds_data["min_lat"]) * lat_km_per_deg
            lon_range_km = (bounds_data["max_lon"] - bounds_data["min_lon"]) * lon_km_per_deg
        ...
        import traceback
        logger.error(f"Error testing tile generation: {str(e)}")
        logger.error(traceback.format_exc())
        return jsonify({
            'error': f'Failed to generate tiles: {str(e)}'
        }), 500
